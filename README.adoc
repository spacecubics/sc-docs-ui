...
=== Select a Site Variant

The existing Space Cubics styling is the default.
To distinguish an internal documentation site, set `site.keys.ui_variant` in the Antora playbook:
...
The `internal` variant uses an earthy olive navigation pane while retaining the shared layout and branding.
If `ui_variant` is absent or has another value, the default styling is used.
